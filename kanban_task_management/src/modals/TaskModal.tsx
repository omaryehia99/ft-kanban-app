...
const TaskModal: React.FC<TaskModalProps> = ({ colIndex, taskIndex, setIsTaskModalOpen }) => {
  ...
  const onClose = (e: React.MouseEvent<HTMLDivElement, MouseEvent>): void => {
  ...
  return (
    <div
      onClick={onClose}
      className=' fixed right-0 left-0 top-0 px-2 py-4 overflow-scroll z-50 bottom-0 justify-center items-center flex scrollbar-hide bg-[#00000080]'
    >
      {/* Modal Section */}

      <div className=' scrollbar-hide overflow-y-scroll max-h-[95vh] my-auto bg-white dark:bg-[#2b2c37] text-black dark:text-white font-bold shadow-md shadow-[#364e7e1a] max-w-md max-auto w-full px-8 py-8 rounded-xl'>
        ...
        <div className=' mt-3 space-y-2'>
          {subtasks.map((subtask: any, i: number) => {
            return <Subtask index={i} taskIndex={taskIndex} colIndex={colIndex} key={i} />;
          })}
        </div>
